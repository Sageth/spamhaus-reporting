...
- Skips any indicator (sending domain, URL, or URL landing domain) matching the built-in domain allowlist, and skips a message outright when an allowlisted domain authenticated *as the sender* (aligned with `From`)
...
**The whole-message allowlist skip requires alignment with `From`.** A message is skipped outright only when an allowlisted domain both authenticated (MTA-verified DKIM signer, or SPF-passing envelope-from) *and* aligns with the `From` domain — the same identity test DMARC applies. Authenticating is not the same as being the sender: every hop on the delivery path authenticates for itself, so a Gmail account with "forward all mail" turned on produces `spf=pass` and `dkim=pass` for `gmail.com` on somebody else's spam. Keying the skip on any authenticated domain would let that forward — or any allowlisted relay, with or without an SRS rewrite — silently veto the report. Matching a merely *claimed* `From` is of course not enough either, or `From: x@paypal.com` would be a free bypass; both halves are required.
